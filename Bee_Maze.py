# -*- coding: utf-8 -*-
"""
Created on Tue Apr 16 23:18:59 2019
...
r = eval(input("Input row number:"))
c = eval(input("Input col number:"))
h = eval(input("How many honey:"))
br = eval(input("Bee row:"))
bc= eval(input("Bee col:"))
MAP = np.random.choice(['x','.'],(r,c))
#蜜蜂位置
MAP[br-1,bc-1]='b' 
#隨機取蜂蜜的所在位置
list1= [ir for ir in range(r)]
list2= [ir for ir in range(c)]
k=MAP[MAP=='h']

while len(k)!=h:
    g=random.sample(list1,h)
    f=random.sample(list2,h)
    for ii in range(0,h):
        if MAP[g[ii],f[ii]]!='b':
            MAP[g[ii],f[ii]]='h'
    k = MAP[MAP=='h']
...
# if D=1 D2 = sqrt(2) → octile distance
         return int(np.abs(self.row - other.row)+ np.abs(self.col - other.col) 
                    - min(np.abs(self.row - other.row),np.abs(self.col - other.col)))
#------------------------------------------------------------------------------------------------
...
class Hive(object):
    def __init__(self, Map, size):
        #為一開始生成的矩陣加上牆壁
        self.sizeRow=np.size(MAP,0)+2 
        self.sizeCol=np.size(MAP,1)+2
        self.bee=Bee(0, 0)
        self.honey=[]
        transfer=[]
        self.map=[]
